...
def generateQuestions(filepath):
    ...
    def generate_mcqs(text, num_questions=10):
        prompt = (
            f"Create {num_questions} multiple-choice questions, and choose a Course Learning Outcome (CLOs) for each question based on the question and I want you to use only three different CLOs, also assign each question a difficulty between 1 and 3, 3 being the hardest. only using the following text:\n\n{text}\n\nFormat: Questions,the the questions and four options (A, B, C, D), and the correct answer as the letter (A, B, C, D), and the CLO, and a difficulty as a number in range of 3."
...
6. **What is the most common measure used to evaluate K-means clusters?**
   ...
   - **Difficulty:** 3"""
    ...
    main(filepath, output_json_path)
